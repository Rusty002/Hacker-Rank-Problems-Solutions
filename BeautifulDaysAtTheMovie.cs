...
class Result
{
    ...
    public static int beautifulDays(int i, int j, int k)
    {
        var days = 0;
        for(int p = i; p <= j; p++)
            if(IsDayBeautiful(p, k))
                days++;
        return days;
    }
    
    public static bool IsDayBeautiful(int number, int k)
    {
        char[] numberArray = number.ToString().ToCharArray();
        Array.Reverse(numberArray);
        string reversedString = new string(numberArray);
        var reverseNumber = Convert.ToInt32(reversedString);
        return Math.Abs(reverseNumber - number) % k == 0 ? true : false;
    }
}
